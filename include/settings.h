...
const float BOOSTSIZEMULT =
    2; // how much boost do agents get? when boost neuron is on
const float DIST =
    150; // how far can the eyes see, ears hear, and nose smell on each bot?
const float DIST_GROUPING =
    40; // how close must another agent be to get grouping health gain
const float EYE_SENSITIVITY = 2;   // how sensitive are the eyes?
const float BLOOD_SENSITIVITY = 2; // how sensitive are blood sensors?
const float METAMUTRATE1 =
    0.002; // what is the change in MUTRATE1 and 2 on reproduction
const float METAMUTRATE2 = 0.05;
const int OLD_AGE_THRESHOLD =
    500; // at what age do they start lossing health for being old?
...
const float LOSS_TEMP =
    0.00005; // loss of health from temperature distribution across world
const float LOSS_AGE = 0.00005; // loss of health from old age
const float GAIN_GROUPING =
    0.00003; // addition of health for each bot near it, as a ratio of closeness
             // (thermal sharing)
...
const float FOOD_MEAT_VALUE =
    0.75; // percentage that health is transferred to another agent when eaten

const float FOODTRANSFER = 0.001; // how much is transfered between two agents
                                  // trading food? per iteration
const float FOOD_SHARING_DISTANCE =
    40; // how far away is food shared between bots?

const float FOOD_DISTRIBUTION_RADIUS =
    100; // when bot is killed, how far is its body distributed?
