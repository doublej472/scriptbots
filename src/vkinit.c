...
// ---- Forward decls ----
static void stage_to_device(VKState *vk, const void *data, VkDeviceSize size, VkBufferUsageFlags usage, VkBuffer *buf,
                            VkDeviceMemory *mem);
...
static void create_buffer(VKState *vk, VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags props,
...
static void stage_to_device(VKState *vk, const void *data, VkDeviceSize size, VkBufferUsageFlags usage, VkBuffer *buf,
  ...
  create_buffer(vk, size, usage | VK_BUFFER_USAGE_TRANSFER_DST_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, buf, mem);

  VkCommandBufferAllocateInfo cbai = {
      .sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO,
      .commandPool = vk->cmd_pool_transfer,
  ...
  if (srcFam == dstFam)
    srcFam = dstFam = VK_QUEUE_FAMILY_IGNORED;
  ...
  vkCmdPipelineBarrier(tmpCmd, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_BOTTOM_OF_PIPE_BIT, 0, 0, NULL, 1,
                       &bmb, 0, NULL);
  vkEndCommandBuffer(tmpCmd);

  VkSubmitInfo si = {.sType = VK_STRUCTURE_TYPE_SUBMIT_INFO, .commandBufferCount = 1, .pCommandBuffers = &tmpCmd};
...
#define CIRCLE_SEGMENTS 32
  ...
  stage_to_device(vk, verts, sizeof(verts), VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, &vk->mesh_circle_vb,
                  &vk->mesh_circle_mem);
}
...
static void create_lines_mesh(VKState *vk) {
  ...
  stage_to_device(vk, verts, sizeof(verts), VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, &vk->mesh_lines_vb, &vk->mesh_lines_mem);
}
...
static void create_hud_mesh(VKState *vk) {
  ...
  stage_to_device(vk, quads, sizeof(quads), VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, &vk->mesh_hud_vb, &vk->mesh_hud_mem);
}
...
#define MAKE_STAGE(flag, mod)                                                                                          \
  (VkPipelineShaderStageCreateInfo){                                                                                   \
      .sType = VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO, .stage = flag, .module = mod, .pName = "main"}

#define CREATE_PIPE(name, vertMod, fragMod, asm, bind, battr, bcount, acount)                                          \
  do {                                                                                                                 \
  ...
  CREATE_PIPE(vk->pipe_circle, circleVert, circleFrag, asmInfo, circleBindings, circleAttrs, 1, 1);
  CREATE_PIPE(vk->pipe_lines, linesVert, linesFrag, asmLine, lineBindings, lineAttrs, 1, 1);
  CREATE_PIPE(vk->pipe_hud, hudVert, hudFrag, asmStrip, hudBindings, hudAttrs, 1, 2);
  CREATE_PIPE(vk->pipe_food, foodVert, foodFrag, asmList, NULL, NULL, 0, 0); // procedural quad — no vertex input
...
static int init_core(VKState *vk) {
  ...
  if (surfaceExt[0] == '\0') {
    ...
    for (uint32_t i = 0; i < glfwExtCount; i++)
      exts[i] = glfwExts[i];
    if (haveDebugUtils)
      exts[glfwExtCount] = VK_EXT_DEBUG_UTILS_EXTENSION_NAME;
    ...
    uint32_t ourCount = 2;
    if (haveDebugUtils)
      ourExts[ourCount++] = VK_EXT_DEBUG_UTILS_EXTENSION_NAME;
    ...
    VkInstanceCreateInfo ici = {
        .sType = VK_STRUCTURE_TYPE_INSTANCE_CREATE_INFO,
        .pApplicationInfo = &appInfo,
        .enabledExtensionCount = ourCount,
        .ppEnabledExtensionNames = ourExts,
        .enabledLayerCount = 1,
        .ppEnabledLayerNames = layers,
        .pNext = haveDebugUtils ? &dbgInfo : NULL,
    };
  ...
  // work can run on separate hardware engines (SDMA, ACE) concurrently.
  {
    ...
    // Save timestamp capabilities for query pool creation after device init
  ...
  // Create device (up to 3 distinct queue families)
  {
    ...
    // GPU timestamp queries — double-buffered: 8 slots (2 × 4)
    // Frame N writes to set [N%2], reads from set [1-N%2] (previous frame, guaranteed ready)
    {
      ...
      vk->timestamp_supported = (vkCreateQueryPool(vk->device, &qpci, NULL, &vk->timestamp_pool) == VK_SUCCESS);
    }
  }
  return 1;
}
...
VKState *vkinit_create(GLFWwindow *window) {
  ...
  // Command pools (one per queue family) & per-frame graphics command buffers
  {
    ...
    VkCommandBufferAllocateInfo cbai = {
        .sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO,
        .commandPool = vk->cmd_pool_gfx,
        .level = VK_COMMAND_BUFFER_LEVEL_PRIMARY,
        .commandBufferCount = 2,
    };
    vkAllocateCommandBuffers(vk->device, &cbai, vk->cmd_buf);

    // Reset all 8 timestamp queries so every slot is initialised before first use.
    // Without this, reading the "previous" slot set on frame 1 trips VUID 09401.
    if (vk->timestamp_supported) {
      ...
      vkFreeCommandBuffers(vk->device, vk->cmd_pool_gfx, 1, &tsCmd);
    }
  }
  ...
  create_buffer(vk, sizeof(CameraUBO), VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
                VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, &vk->cam_ubo_buf,
                &vk->cam_ubo_mem);
  // Agent SSBO — sized for initial population, resizeable on demand
  vk->agent_capacity = (uint32_t)NUMBOTS;
  if (vk->agent_capacity < 65536)
    vk->agent_capacity = 65536; // floor 64K
  create_buffer(vk, vk->agent_capacity * sizeof(AgentInstance), VK_BUFFER_USAGE_STORAGE_BUFFER_BIT,
                VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, &vk->agent_buf,
                &vk->agent_mem);
  create_buffer(vk, (VkDeviceSize)FOOD_SQUARES_WIDTH * FOOD_SQUARES_HEIGHT * sizeof(float),
                VK_BUFFER_USAGE_STORAGE_BUFFER_BIT,
                VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, &vk->food_data_buf,
                &vk->food_data_mem);
...
void vkinit_destroy(VKState *vk) {
  ...
  vkbrain_destroy(vk);
  if (vk->timestamp_pool)
    vkDestroyQueryPool(vk->device, vk->timestamp_pool, NULL);
...
void vkdraw_resize_agents(VKState *vk, uint32_t new_capacity) {
  ...
  create_buffer(vk, new_capacity * sizeof(AgentInstance), VK_BUFFER_USAGE_STORAGE_BUFFER_BIT,
                VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, &vk->agent_buf,
                &vk->agent_mem);
  vkMapMemory(vk->device, vk->agent_mem, 0, VK_WHOLE_SIZE, 0, (void **)&vk->mapped_agents);
