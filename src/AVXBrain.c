...
void avxbrain_print(struct AVXBrain* b) {
	...
	for (size_t i = 0; i < BRAIN_DEPTH; i++) {
		...
		for (size_t j = 0; j < BRAIN_WIDTH; j++) {
			...
			sprintf(buffer, "brain[%zu].inputs[%zu]=[", i, j);
			for (size_t k = 0; k < BRAIN_ELEMENTS_PER_VECTOR; k++) {
				char buffer2[2048];
				sprintf(buffer2, "%f ", b->layers[i].inputs[j].f32[k]);
		...
		for (size_t j = 0; j < BRAIN_WIDTH; j++) {
			...
			sprintf(buffer, "brain[%zu].biases[%zu]=[", i, j);
			for (size_t k = 0; k < BRAIN_ELEMENTS_PER_VECTOR; k++) {
				char buffer2[2048];
				sprintf(buffer2, "%f ", b->layers[i].biases[j].f32[k]);
				strcat(buffer, buffer2);
			}
			strcat(buffer, "]\n");
		...
		for (size_t j = 0; j < BRAIN_WEIGHTS; j++) {
			...
			sprintf(buffer, "brain[%zu].weights[%zu]=[", i, j);
			for (size_t k = 0; k < BRAIN_ELEMENTS_PER_VECTOR; k++) {
				char buffer2[2048];
				sprintf(buffer2, "%f ", b->layers[i].weights[j].f32[k]);
				strcat(buffer, buffer2);
			}
			strcat(buffer, "]\n");
...
void avxbrain_init_random(struct AVXBrain* b) {
	...
	for (size_t i = 0; i < BRAIN_DEPTH; i++) {
		...
		for (size_t j = 0; j < BRAIN_WIDTH; j++) {
			// Set biases
			for (size_t k = 0; k < BRAIN_ELEMENTS_PER_VECTOR; k++) {
				b->layers[i].biases[j].f32[k] = (SDL_randf() - 0.5f) * BRAIN_BIAS_RANGE * 2;
			}
		}

		for (size_t j = 0; j < BRAIN_WEIGHTS; j++) {
			for (size_t k = 0; k < BRAIN_ELEMENTS_PER_VECTOR; k++) {
				b->layers[i].weights[j].f32[k] = (SDL_randf() - 0.5f) * BRAIN_WEIGHT_RANGE * 2;
			}
		}
	}
}
...
void avxbrain_tick(
	struct AVXBrain* b,
	float (*brain_inputs)[BRAIN_INPUT_SIZE],
	float (*brain_outputs)[BRAIN_OUTPUT_SIZE]
) {
	// Set the inputs for the first layer
	for (int i = 0; i < BRAIN_INPUT_SIZE; i++) {
		b->layers[0].inputs[i / BRAIN_ELEMENTS_PER_VECTOR].f32[i % BRAIN_ELEMENTS_PER_VECTOR] = (*brain_inputs)[i];
	}
	...
	for (size_t i = 0; i < BRAIN_DEPTH; i++) {
		...
		for (size_t j = 0; j < BRAIN_WIDTH_ELEMENTS; j++) {
			...
			simde__m512 input = simde_mm512_set1_ps(layer->inputs[j / BRAIN_ELEMENTS_PER_VECTOR].f32[j % BRAIN_ELEMENTS_PER_VECTOR]);

			// For each group of weights
			for (size_t k = 0; k < BRAIN_WIDTH; k++) {
				sum[k] = simde_mm512_add_ps(simde_mm512_mul_ps(
					input,
					layer->weights[(j * BRAIN_WIDTH) + k]
				),
					sum[k]
				);
			}
		...
		if (i == BRAIN_DEPTH - 1) {
			for (size_t j = 0; j < BRAIN_OUTPUT_SIZE; j++) {
				(*brain_outputs)[j] = sum[j / BRAIN_ELEMENTS_PER_VECTOR].f32[j % BRAIN_ELEMENTS_PER_VECTOR];
			}
		}
		else {
			for (size_t j = 0; j < BRAIN_WIDTH; j++) {
...
void avxbrain_mutate(struct AVXBrain* b, float mutaterate, float mutaterate2) {
	...
	for (size_t i = 0; i < BRAIN_DEPTH; i++) {
		...
		for (size_t j = 0; j < BRAIN_WIDTH; j++) {
			for (size_t k = 0; k < BRAIN_ELEMENTS_PER_VECTOR; k++) {
				if (SDL_randf() < mutaterate) {
					float in = b->layers[i].biases[j].f32[k];
					in += (SDL_randf() - 0.5f) * mutaterate2 * 2;
		...
		for (size_t j = 0; j < BRAIN_WEIGHTS; j++) {
			for (size_t k = 0; k < BRAIN_ELEMENTS_PER_VECTOR; k++) {
				if (SDL_randf() < mutaterate) {
					float in = b->layers[i].weights[j].f32[k];
					in += (SDL_randf() - 0.5f) * mutaterate2 * 2;
