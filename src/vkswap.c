...
static void create_swapchain(VKState *vk) {
  ...
  if (extent.width == UINT32_MAX) {
    int w, h;
    glfwGetFramebufferSize(vk->window, &w, &h);
...
void vkswap_create(VKState *vk) { create_swapchain(vk); }
...
static void destroy_swapchain_resources(VKState *vk) {
  ...
  free(vk->sc_framebufs);
  free(vk->sc_views);
  free(vk->sc_images);
  vk->sc_framebufs = NULL;
  vk->sc_views = NULL;
  vk->sc_images = NULL;
  vk->sc_count = 0;
}
...
void vkswap_get_extent(VKState *vk, uint32_t *w, uint32_t *h) {
  *w = vk->sc_extent.width;
  *h = vk->sc_extent.height;
}
