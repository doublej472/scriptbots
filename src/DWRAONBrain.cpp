...
DWRAONBrain DWRAONBrain::crossover(const DWRAONBrain &other) {
  ...
  for (int i = 0; i < newbrain.boxes.size(); i++) {
    newbrain.boxes[i].bias =
        randf(0, 1) < 0.5 ? this->boxes[i].bias : other.boxes[i].bias;
    newbrain.boxes[i].kp =
        randf(0, 1) < 0.5 ? this->boxes[i].kp : other.boxes[i].kp;
    newbrain.boxes[i].type =
        randf(0, 1) < 0.5 ? this->boxes[i].type : other.boxes[i].type;

    for (int j = 0; j < newbrain.boxes[i].id.size(); j++) {
      newbrain.boxes[i].id[j] =
          randf(0, 1) < 0.5 ? this->boxes[i].id[j] : other.boxes[i].id[j];
      newbrain.boxes[i].notted[j] = randf(0, 1) < 0.5
                                        ? this->boxes[i].notted[j]
                                        : other.boxes[i].notted[j];
      newbrain.boxes[i].w[j] =
          randf(0, 1) < 0.5 ? this->boxes[i].w[j] : other.boxes[i].w[j];
    }
  }
  return newbrain;
}
