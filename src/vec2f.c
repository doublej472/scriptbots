...
float vector2f_length(struct Vector2f *vec) { return sqrtf(vec->x * vec->x + vec->y * vec->y); }

float vector2f_dist(struct Vector2f *vec1, struct Vector2f *vec2) { return sqrtf(vector2f_dist2(vec1, vec2)); }
